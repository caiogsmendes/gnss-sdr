...
glonass_l2_ca_telemetry_decoder_gs::~glonass_l2_ca_telemetry_decoder_gs()
{
    // D// LOG(INFO) << "Glonass L2 Telemetry decoder block (channel " << d_channel << ") destructor called.";
    size_t pos = 0;
            ...
            if (abs(corr_value) >= d_symbols_per_preamble)
                {
                    ...
                    if (abs(preamble_diff - GLONASS_GNAV_PREAMBLE_PERIOD_SYMBOLS) == 0)
                    ...
                    else
                        {
                            ...
                            // D// LOG(INFO) << "Failed string decoder for GLONASS L2 C/A SAT " << this->d_satellite;
                        }
                }
        }
    else if (d_stat == 2)
