...
glonass_l1_ca_telemetry_decoder_gs::~glonass_l1_ca_telemetry_decoder_gs()
{
    // D// LOG(INFO) << "Glonass L1 Telemetry decoder block (channel " << d_channel << ") destructor called.";
    size_t pos = 0;
            ...
            if (abs(corr_value) >= d_symbols_per_preamble)
                {
                    ...
                    if (abs(preamble_diff - GLONASS_GNAV_PREAMBLE_PERIOD_SYMBOLS) == 0)
                    ...
                    else
                        {
                            ...
                            // D// LOG(INFO) << "Failed string decoder for GLONASS L1 C/A SAT " << this->d_satellite;
                        }
                }
        }
    else if (d_stat == 2)
                            ...
                            if (!d_flag_frame_sync)
                                {
                                    ...
                                    // D// LOG(INFO) << " Frame sync SAT " << this->d_satellite << " with preamble start at "
