    ...
    if (samples_ == 0)  // read all file
        {
            ...
            if (file.is_open())
                {
                    size = file.tellg();
                    // D// LOG(INFO) << "Total samples in the file= " << floor(static_cast<double>(size) / static_cast<double>(item_size()));
                }
            else
                {
            ...
            for (int32_t n = 0; n < n_channels_; n++)
                {
                    ...
                    // D// LOG(INFO) << "connected file_source #" << n << " to throttle";
                    top_block->connect(throttle_vec_.at(n), 0, valve_, n);
                    // D// LOG(INFO) << "connected throttle #" << n << " to valve_";
                }
        }
    else
        {
            for (int32_t n = 0; n < n_channels_; n++)
                {
                    ...
                    // D// LOG(INFO) << "connected file_source #" << n << " to valve_";
                }
        }
}
            ...
            for (int32_t n = 0; n < n_channels_; n++)
                {
                    ...
                    // D// LOG(INFO) << "disconnected file_source #" << n << " to throttle";
                    top_block->disconnect(throttle_vec_.at(n), 0, valve_, n);
                    // D// LOG(INFO) << "disconnected throttle #" << n << " to valve_";
                }
        }
    else
        {
            for (int32_t n = 0; n < n_channels_; n++)
                {
                    ...
                    // D// LOG(INFO) << "disconnected file_source #" << n << " to valve_";
                }
        }
}
